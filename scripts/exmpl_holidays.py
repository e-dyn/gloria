...
if __name__ == "__main__":
    ...
    model.add_external_regressor('ano_deviation', prior_scale = 0.3, mode = 'additive')
    ...
    if INCLUDE_SERIALIZATION_STEP:
        ...
        # And load it
        model_new = Gloria.from_json(model_json = model_path,
                                 return_as = 'model')
    
    data = model.make_future_dataframe(periods = 30)
    data = pd.concat([data, df['ano_deviation']], axis = 1).reset_index(drop = True)
