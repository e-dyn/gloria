...
### --- Module Imports --- ###
# Standard Library
from typing import Optional, Type, Any, Literal, Union, TYPE_CHECKING
...
from gloria.constants import (_HOLIDAY, _EVENT_PRIOR_SCALE, _EVENT_MODE,
...
def get_holidays(
    ...
    if not hasattr(holidays, country):
        raise AttributeError(f"Holidays in {country} are not currently "
                             "supported!")
    ...
    if timestamps is None:
        from holidays.constants import DEFAULT_START_YEAR, DEFAULT_END_YEAR
        years = range(DEFAULT_START_YEAR,DEFAULT_END_YEAR+1)
    else:
...
def make_holiday_dataframe(
    ...
    for name in all_holiday_names:
        holiday_df_loc = pd.DataFrame({
            timestamp_name: all_holidays.get_named(name),
            _HOLIDAY: name
        })
...
class Holiday(IntermittentEvent):
    ...
    def make_feature(
        ...
        self.t_list = (holiday_df[t_name]
                       .loc[holiday_df[_HOLIDAY] == self.name].values)
        
        # Once we have the list, the IntermittentEvent.make_feature() method
        # will take care of the rest.
        return super().make_feature(timestamps)
...
class CalendricData(Protocol):
    ...
    country: str
    subdiv: Optional[str] = None
    holiday_mode: Literal['additive', 'multiplicative'] = _EVENT_MODE
    holiday_prior_scale: float = Field(gt = 0, default = _EVENT_PRIOR_SCALE)
    holiday_event: Event = BoxCar(duration = '1d')
    ...
    @field_validator(*(s + '_seasonality' 
                       for s in DEFAULT_SEASONALITIES.keys()))
    @classmethod
    def validate_seasonality_arg(
            cls: Type[Self],
            arg: Union[bool, str, int]
        ) -> Union[bool, str, int]:
        """
    ...
    def set_seasonalities(
        ...
        for season, prop in DEFAULT_SEASONALITIES.items():
            ...
            if add_mode is True:
                 fourier_order = prop['default_order']
                ...
                if timespan / prop["period"] < 2:
                    continue
                # Maximum order fulfilling Nyquist sampling condition
                max_order = int(np.floor(
                    prop["period"] / (2*inferred_sampling_period)
                ))
                # add orders up to default_order but no higher than max_order
                fourier_order = min(prop['default_order'], max_order)
