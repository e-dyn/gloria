...
# Inhouse Packages
from cassandra.constants import _DELIM
...
class AbstractRegressor(BaseModel, ABC):
    """
    Base class for adding regressors to the Cassandra model and creating the
...
class ExternalRegressor(AbstractRegressor):
    """
    Used to add external regressors to the Cassandra model and create its
    feature matrix
    """
...
class Seasonality(AbstractRegressor):
    """
    Used to add a seasonality regressors to the Cassandra model and create its
    feature matrix
    
    Important: Period is unitless. That is, when called from Cassandra, it will
...
class Holidays(AbstractRegressor):
    ...
    TODO:
        - t0 und sampling_delta als Parameter der make_feature Methode, da
          diese vom Cassandra Modell und Trainingsdaten abhängen
