"""
TODO:
    - n_changepoints = 0 or 1 doesn't work properly'
...
from cassandra.constants import _T_CONVERSION, _DELIM, _T_INT
from cassandra.models import ModelInputData, MODEL_MAP, ModelBackend
from cassandra.regressors import ExternalRegressor, Seasonality
...
class Cassandra(BaseModel):
    ...
    def add_seasonality(
        ...
        """
        Add an seasonality to be used for fitting and predicting.
    ...
    def fit(
        ...
        **kwargs : dict[str, Any]
        ...
        """
        if self.history is not None:
            raise Exception('Cassandra object can only be fit once. '
    ...
    def predict(self, timestamps: pd.DataFrame) -> pd.DataFrame:
        """
        Predict using the fitted Cassandra model.
